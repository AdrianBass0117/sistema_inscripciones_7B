    ...
    <div id="sendNotificationModal" class="modal">
        <div class="modal-content large">
            ...
            <div class="modal-body">
                <form id="notificationForm" action="{{ route('comite.notificaciones.store') }}" method="POST">
                    @csrf
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (markAllReadBtn) {
                markAllReadBtn.addEventListener('click', function() {
                    if (confirm(
                        ...
                        fetch('{{ route('comite.notificaciones.markAllRead') }}', {
                                method: 'POST',
                                headers: {
            ...
            function markAsRead(notificationId, notification, btn) {
                fetch('{{ route('comite.notificaciones.markAsRead') }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            function markAsUnread(notificationId, notification, btn) {
                fetch('{{ route('comite.notificaciones.markAsUnread') }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            if (loadMoreBtn) {
                loadMoreBtn.addEventListener('click', function() {
                    ...
                    fetch(`{{ route('comite.notificaciones') }}?page=${page}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.html) {
