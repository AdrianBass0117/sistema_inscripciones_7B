...
@section('content')
    <div class="notificaciones-content">
        ...
        <div class="notifications-list" id="notificationsList">
            ...
            <div class="notification-item {{ $notificacion->leida ? 'read' : 'unread' }} {{ $notificacion->tipo === 'urgente' ? 'important' : '' }}"
                 ...
                 data-date="{{ $notificacion->created_at->format('Y-m-d') }}">
                ...
                <div class="notification-icon {{ $notificacion->tipo === 'urgente' ? 'urgent' : ($notificacion->tipo === 'recordatorio' ? 'warning' : 'info') }}">
                    <i class="fas {{ $notificacion->tipo === 'urgente' ? 'fa-exclamation-triangle' : ($notificacion->tipo === 'recordatorio' ? 'fa-clock' : 'fa-info-circle') }}"></i>
                </div>
                ...
                <div class="notification-content">
                    ...
                    <div class="notification-meta">
                        <span class="notification-category {{ $notificacion->tipo }}">{{ $notificacion->getTipoFormateado() }}</span>
                        <span class="notification-destinatarios">{{ $notificacion->getDestinatariosFormateado() }}</span>
                        <span class="notification-time-full">{{ $notificacion->created_at->format('d M, h:i A') }}</span>
                    </div>
                </div>

                <div class="notification-actions">
                    @if(!$notificacion->leida)
                    <button class="btn-action mark-read" title="Marcar como leída" data-id="{{ $notificacion->id_notificacion }}">
                        <i class="fas fa-envelope-open"></i>
                    </button>
                    @else
                    <button class="btn-action mark-unread" title="Marcar como no leída" data-id="{{ $notificacion->id_notificacion }}">
                        <i class="fas fa-envelope"></i>
                    </button>
                    @endif
                </div>
   ...
   <style>
        ...
        .notification-category.inscriptions {
            background: rgba(0, 170, 139, 0.1);
            color: var(--secondary-color);
        }
        ...
        .btn-action.mark-unread {
            padding: 0.5rem;
            min-width: 40px;
            justify-content: center;
        }

        .btn-action.urgent-action {
            background: var(--error);
            border-color: var(--error);
            color: white;
        }

        .btn-action.urgent-action:hover {
            background: #C53030;
            border-color: #C53030;
        }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (markAllReadBtn) {
                markAllReadBtn.addEventListener('click', function() {
                    if (confirm('¿Estás seguro de que quieres marcar todas las notificaciones como leídas?')) {
                         fetch('{{ route("aspirante.notificaciones.markAllRead") }}', {
                            method: 'POST',
                            headers: {
            ...
            function markAsRead(notificationId, notification, btn) {
                fetch('{{ route("aspirante.notificaciones.markAsRead") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            function markAsUnread(notificationId, notification, btn) {
                fetch('{{ route("aspirante.notificaciones.markAsUnread") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
            ...
            if (loadMoreBtn) {
                loadMoreBtn.addEventListener('click', function() {
                   ...
                   fetch(`{{ route("aspirante.notificaciones") }}?page=${page}`)
                        .then(response => response.json())
                        .then(data => {
                            if (data.html) {
